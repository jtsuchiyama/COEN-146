...
// Date: 2/9/22
// Title: Lab6 - Setting up a UDP Client w/ Reliable Transfer and Loss Prevention
// Description: The program sets up a client that sends a source file to the server using the UDP protocol with reliable transfer. 
// This is achieved by implementing the stop and wait protocol with a checksum, sequence number, and acknowledgement number.
// The server is targeted by declaring the server IP and port. While the client is running, it simulates the corruption of the checksum 
// and reads the results of the ACK. Furthermore, the client also simulates the dropping of the packet.   
...
void clientSend(int sockfd, const struct sockaddr *address, socklen_t addrlen, Packet packet) {
	...
	while (1) 
	{
		// if retries is greater than 3, we give up
		if(retries >= 3)
			break;
		...
		if(rand() % 5 == 0) // simulate a drop of a packet at 20%
			printf("Dropping packet\n");
		
		else // 80% chance to not drop packet
		{
        		//send the packet
		...
		// sets up a read file descriptor at the beginning of the function to be checked for being ready to read
		fd_set readfds;
